...
elseif (strcmpi(display_criteria, 'group comparisons'))
    ...
    if (exist('meta_states_info', 'var'))
        ...
        for nMeta = 1:length(fNs_meta)
            ...
            if (exist('tmp_nuisance', 'var'))
                % regress covariates
                tmp = regress_cov(tmp, tmp_nuisance);
            end
...
elseif strcmpi(display_criteria, 'tvdfnc analysis')
    ...
    tvdfnc_centroids = tvdfncInfo.Call{1};
    tvdfnc_centroid_derivatives = tvdfncInfo.Call{2};
    
    %tvdfnc_centroid_derivatives = tvdfnc_centroids(:, (size(tvdfnc_centroids,2)/2) + 1:end);
    %tvdfnc_centroids = tvdfnc_centroids(:, 1:size(tvdfnc_centroids,2)/2);
    H = [];
    for ii = 1:dfncInfo.postprocess.num_clusters
        tmpStr = sprintf('%d (%d%%)', sum(tvdfncInfo.IDXall == ii),  round(100*sum(tvdfncInfo.IDXall == ii)/length(tvdfncInfo.IDXall)));
        titleStr = ['TVdFNC Cluster Centroid (', num2str(ii), ')'];
...
elseif strcmpi(display_criteria, 'task connectivity')
    ...
    for ii = 1:length(dfncInfo.outputFiles)
        load(fullfile(outputDir, dfncInfo.outputFiles{ii}), 'task_connectivity');
        if (ii == 1)
            taskMean = zeros(size(task_connectivity));
        end
        taskMean = taskMean + task_connectivity;
    end
    taskMean = taskMean ./ M;
    CLIM = max(abs(taskMean(:)));
    CLIM = [-CLIM, CLIM];
    
    for ii = 1:length(dfncInfo.userInput.selectedRegressors)
        fig_title = ['Task connectivity (', dfncInfo.userInput.selectedRegressors{ii},')'];
        H(ii).H = icatb_getGraphics(fig_title, 'graphics', 'dfnc_summary6', figVisible);
        colormap(jet);
        sh = axes('units', 'normalized', 'position', [0.2, 0.2, 0.6, 0.6]);
        ...
        if (nargout == 1)
            outFile = [dfncInfo.prefix, '_task_connectivity_', dfncInfo.userInput.selectedRegressors{ii}, '.jpg'];
            printFile(H(ii).H, fullfile(html_dir, outFile));
            results(ii).file = outFile;
            results(ii).text = fig_title;
        end
